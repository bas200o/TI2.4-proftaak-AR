...
cv::Mat Vision::ColorDetection::colorDetection(cv::Mat frame)
{
	...
	inRange(hsvImage, cv::Scalar(34 - 7, 255 - 30, 205 - 73), cv::Scalar(34 + 7, 255 + 30, 205 + 73), mask);
...
std::vector<cv::Point2f> Vision::ColorDetection::trackObject(cv::Mat frame, cv::Mat threshold)
{
	cv::Mat temp;
...
void Vision::ColorDetection::checkIfInROI(cv::Rect rec, cv::Rect rec2, cv::Rect rec3, cv::Rect rec4)
{
